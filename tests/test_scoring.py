...
from marginscout_public.scoring import OpportunitySignals, score_opportunity  # noqa: E402
...
class OpportunityScoringTests(unittest.TestCase):
    ...
    def test_keyword_match_without_request_intent_is_capped(self) -> None:
        result = score_opportunity(
            OpportunitySignals(
                request_intent=Decimal("10"),
        ...
        self.assertIn("service-request intent is not established", result.applied_caps)

    def test_out_of_range_signal_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            score_opportunity(
                OpportunitySignals(
                    request_intent=Decimal("101"),
